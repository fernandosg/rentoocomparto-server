...
class PlacesController extends Controller {
    ...
    */
    public function by_characteristic(Request $request){
      $m = self::MODEL;
      $places=$m::where("city_id",$request->input("city_id"))->where("price",">=",$request->input("min_price"))->where("price","<=",$request->input("max_price"))->where("type_id",$request->input("type_id"))->with(["type","city","offer"])->get();
      return response()->json($places);
    }
